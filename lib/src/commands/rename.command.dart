...
Future<void> onRenameCommand(
    nyxx.MessageReceivedEvent event, List<String>? args) async {
  try {
    /// args is null or args length is not equal to 3, return.
    if (args == null) return;

    nyxx.Guild? guild =
        (event.message as nyxx.GuildMessage).guild.getFromCache()!;
    if (args[0] != 'all') {
      nyxx.Message renameWaitingMsg =
          await event.message.channel.sendMessage(MessageContent.waiting);
      nyxx.Role? role = (event.message as nyxx.GuildMessage)
          .roleMentions
          .first
          .getFromCache();
      nyxx.GuildPreview wtf = await guild.fetchGuildPreview();
      Stream<nyxx.Member?> allMembers =
      ...
      await allMembers.forEach((nyxx.Member? mem) async {
        ...
        if (!mem.user.getFromCache()!.bot) {
          if (mem.nickname != null) {
            for (nyxx.Cacheable<nyxx.Snowflake, nyxx.Role> guildRole
                in mem.roles) {
      ...
      await event.message.channel.sendMessage(MessageContent.renameDone());
    } else if (args[0] == 'all') {}
  } catch (e) {
