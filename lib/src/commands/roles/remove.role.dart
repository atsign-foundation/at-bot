...
Future<void> removeRoleToUser(nyxx.IMessageReceivedEvent event,
  ...
  try {
    ...
    if (args[1].startsWith('<@!') && args[1].endsWith('>')) {
      user = event.message.mentions.first.getFromCache();
    ...
    if (userHasRole) {
      ...
      await member.edit(builder: nyxx.MemberBuilder()..nick = memNickName);
      ...
      await logAndSendMessage(
          ...
          MessageContent.custom(
              'No role found in role list of ${user.username}.'),
          LogTypeTag.warning,
          'No role found in role list of ${user.username}.  -by ${event.message.author.username}');
    }
  } catch (e) {
